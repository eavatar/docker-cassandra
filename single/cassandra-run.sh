...
if [ ! -f /home/ava/.cassconfig ]; then
    ...
    echo "JVM_OPTS=\"\$JVM_OPTS -Dcassandra.initial_token=0\"" >> $ENV_FILE
    echo "JVM_OPTS=\"\$JVM_OPTS -Dcassandra.skip_wait_for_gossip_to_settle=0\"" >> $ENV_FILE
    echo "JVM_OPTS=\"\$JVM_OPTS -Djava.rmi.server.hostname=$ADDR\"" >> $ENV_FILE
    ...
    touch /home/ava/.cassconfig
fi

cd /opt/cassandra
exec chpst -u ava /opt/cassandra/bin/cassandra -f
